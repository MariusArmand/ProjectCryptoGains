    ...
    public static class PrintUtils
    {
            ...
            )
        {
            await Task.Run(() =>
            {
                ...
                // Prefix pdfTitle/title with printout title prefix setting if set
                string pdfTitle = title;
                if (!string.IsNullOrEmpty(SettingPrintoutTitlePrefix))
                {
                    pdfTitle = $"{SettingPrintoutTitlePrefix} {title}";
                    title = $"{SettingPrintoutTitlePrefix} - {title}";
                }
        ...
        private class CustomPaginator : DocumentPaginator
        {
            ...
            public override DocumentPage GetPage(int pageNumber)
            {
                ...
                using (DrawingContext dc = visual.RenderOpen())
                {
                    ...
                    double pageNumberX = (_pageWidth - formattedText.Width) / 2; // Center horizontally
                    double pageNumberY = originalPage.Size.Height - _footerHeight + (_footerHeight - formattedText.Height) / 2; // Center vertically in footer
                    // Draw the page number text onto the page
                    dc.DrawText(formattedText, new Point(pageNumberX, pageNumberY));

                    // Create generatedByText (bottom right)
                    FormattedText generatedByText = new FormattedText(
                        "Generated by ProjectCryptoGains",
                        System.Globalization.CultureInfo.CurrentCulture,
                        FlowDirection.LeftToRight,
                        new Typeface(_fontFamily, FontStyles.Normal, FontWeights.Normal, FontStretches.Normal),
                        _fontSize,
                    ...
                    double rightMargin = 20; // Small margin from the right edge
                    double createdByX = _pageWidth - generatedByText.Width - rightMargin; // Align to right with margin
                    double createdByY = originalPage.Size.Height - _footerHeight + (_footerHeight - generatedByText.Height) / 2; // Same vertical position as page number

                    // Draw the generatedByText text onto the page
                    dc.DrawText(generatedByText, new Point(createdByX, createdByY));
                }
